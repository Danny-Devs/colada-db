...
2. **L4 real-browser lane (DAN-652)** — ⬅️ **NEXT.** Converts mocked durability confidence into real evidence; the publish/grant credibility artifact. L2 sharpened the case for it: the SQLite conformance run uses a `:memory:` database, so its reopen property proves data survives dropping the *engine*, not the *process*. Real process-death on real OPFS is only provable in a browser.
