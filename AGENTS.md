...
- **`pnpm lint` is oxlint PLUS a repo-local rule.** `no-unguarded-process-env` (`scripts/no-unguarded-process-env.mjs`) bans any reference to the global `process` in shipped source unless guarded by `typeof process !== "undefined"` — shipped code must run in bundler-less runtimes (CDN / Deno / plain ESM) where `process` does not exist, and the failure only appears on degradation paths. Spec files are exempt. If it fires, read its message rather than reaching for an ignore: see LESSONS.md 2026-07-23.
