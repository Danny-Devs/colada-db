...
**Implementation:** in-progress (the coordinator — `enableSync()` — landed 2026-08-02 via DAN-776, conformance-tested against all 14 `SYNC_CONTRACT_COVERAGE.coordinator` obligations; still open: the §1 durable outbox and stable-identity reload semantics (DAN-777) and the real adapters (`restAdapter` et al.) this interface exists for)
