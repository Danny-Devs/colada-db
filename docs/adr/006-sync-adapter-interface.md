...
**Implementation:** in-progress (the coordinator — `enableSync()` — landed 2026-08-02 via DAN-776, conformance-tested against all 14 `SYNC_CONTRACT_COVERAGE.coordinator` obligations; **`restAdapter` — the roadmap's first adapter — landed 2026-08-02 via DAN-780**, passing the full adapter kit with every hook supplied against a stub wire-protocol-v1 server, plus an end-to-end `enableSync ↔ restAdapter ↔ HTTP` integration proof; **the §1 durable outbox and stable-identity reload semantics landed 2026-08-02 via DAN-777** (`outboxEngine` on `EnableSyncOptions` — a separate engine instance per §1's own-file/store requirement — plus the pull-apply sibling replay and non-regressing verdict stamps); still open: `electricAdapter`)
...
### Implementation note, part 2 (2026-08-02) — `reject` cannot call `transactions.ts`'s `rollback()`, and `transform`'s "move" is two events, not one

Found while building the coordinator (DAN-776), against the same live tree part 1 was written against.

**§1 says a `reject` verdict "triggers the existing rollback machinery immediately."** `OptimisticTransaction.rollback()` (`src/transactions.ts:129`) is only reachable while the transaction is still in `activeTransactions` — `commit()` splices it out (`transactions.ts:444`) and `doRollback()` no-ops on an index of `-1` (`transactions.ts:361`). A `reject` verdict always arrives from `push()`, which always runs after `commit()` already happened locally (§1: "`commit()` moves its mutations into a durable outbox"). By the time a `reject` can exist, the transaction it would roll back has already settled and is gone. There is no post-commit path back into that machinery, and building one means editing `transactions.ts` — out of scope for a ticket that consumes it read-only.

**The fix does not require touching `transactions.ts`.** `EntityEvent.previousData` (`src/types.ts:186`) already carries what a compensating write needs — captured for free at the moment the outbox tap observes the `local-mutation` event, no new snapshot machinery required. On `reject`, the coordinator writes `previousData` back (or `store.remove()` if `previousData` was `undefined`, meaning the entity didn't exist before this entry) under a NEW write channel — `origin: "undo"`, already reserved in the shipped `WriteOrigin` union (`src/types.ts:150`) and never used by any shipped writer yet. `"undo"` is outside the outbox's `"local-mutation"` accept-list by construction, so a reject's compensating write can never re-enter the outbox — echo suppression falls out of the existing filter with no special-casing.

**Revert-and-replay, not revert-and-stop, when two outbox entries touch the same entity.** If entry A (`w1`, still pending) is followed by entry B (`w1` again, also still pending) and A is rejected, reverting to A's `previousData` alone would silently discard B's edit — B is still an outstanding, unconfirmed local write. The coordinator replays every OTHER still-outstanding outbox entry for that same key, in outbox order, after applying the revert — the same "restore truth, then replay the remaining active mutations" shape `transactions.ts`'s own `recompute()` already uses for its own (pre-commit) rollback, independently arrived at for the post-commit case because the constraint is the same one: an entity with more than one in-flight writer must not let one writer's undo erase another's still-live write.

**`transform`'s "rekey entity... one move event" is, concretely, a remove-then-set pair.** `EntityStore` has no rename/move primitive — `type: "set" | "remove" | "evict"` is the complete event vocabulary (`src/types.ts:176`). A `remappedId` is applied as `store.set(entityType, remappedId, data)` followed by `store.remove(entityType, tempId)`, both under `origin: "sync-pull"` in one `runWith` call so both land as one write-channel unit even though they are two `EntityEvent`s. "One move event" describes the *outbox's* bookkeeping (one logical rename, tracked as such in outbox refs) rather than a single `EntityEvent` — there is no single-event primitive available to build one.
