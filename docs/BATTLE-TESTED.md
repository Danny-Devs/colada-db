...
| **TinyBase** | The `willCommit` veto gate — a validation hook that becomes THE agent-policy enforcement point; transaction-end netting (A→B→A emits nothing) | PLANNED — designed in ADR-007, ships with Stage 2a (not in code yet) |
| **Evolu** | The capped, queryable history store (`{entity, field, old→new, origin}`) — undo substrate that doubles as agent receipts; `local:`-only entity tier | PLANNED — designed in ADR-007, ships with Stage 2a; `sync: false` tier ships with sync (ADR-005) |
