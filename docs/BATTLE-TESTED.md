...
| **TinyBase** | The `willCommit` veto gate — a validation hook that becomes THE agent-policy enforcement point; transaction-end netting (A→B→A emits nothing) | ADR-007; DAN-577 |
| **Evolu** | The capped, queryable history store (`{entity, field, old→new, origin}`) — undo substrate that doubles as agent receipts; `local:`-only entity tier | ADR-007; `sync: false` entities (ADR-005) |
